...
function EnrollmentOrderModal({ onClose }) {
  ...
  const loadEnrolledStudents = async () => {
    ...
    try {
      ...
      setStreamStudents(response.data.students || [])
    } catch (error) {
  ...
  const handleSelectAll = () => {
    if (selectedStudentIds.length === streamStudents.length) {
      ...
      setSelectedStudentIds(streamStudents.map(s => s.id))
    }
  }
    ...
    <div className="modal">
      <div className="modal-content" style={{ maxWidth: '700px' }}>
        ...
        {selectedStream && streamStudents.length > 0 && (
          <>
            <div className="form-group">
              <label>Студенты для зачисления *</label>
        ...
        {selectedStream && streamStudents.length === 0 && (
          <p style={{ color: '#666', fontStyle: 'italic' }}>
            В этом потоке пока нет студентов. Сначала зачислите студентов в поток.
          </p>
        )}
